...
export async function requestHotspotPermissions(): Promise<{ granted: boolean; missing: string[]; blocked: boolean }> {
  if (Platform.OS !== 'android') return { granted: false, missing: ['android only'], blocked: false };
  const api = Platform.Version as number;
  const wanted: string[] = api >= 33 ? ['android.permission.NEARBY_WIFI_DEVICES'] : ['android.permission.ACCESS_FINE_LOCATION'];
  const results = (await PermissionsAndroid.requestMultiple(wanted as never[])) as Record<string, string>;
  const missing = wanted.filter((p) => results[p] !== PermissionsAndroid.RESULTS.GRANTED);
  const blocked = missing.some((p) => results[p] === PermissionsAndroid.RESULTS.NEVER_ASK_AGAIN);
  return { granted: missing.length === 0, missing: missing.map((m) => m.replace('android.permission.', '')), blocked };
}
