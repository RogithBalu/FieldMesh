...
- Hotspot join fails → the joiner can connect to the host's SSID manually from Wi-Fi settings (use the passphrase on the host's screen), then tap "Retry joining". "Retry joining" re-does the Wi-Fi join, not just the socket; "Cancel and clear" resets the card so you can host or scan again. With a single phone and no host there is nothing to join — that error is expected.
